...
WHERE joindate = (
    SELECT MAX(joindate) AS lastjoin_datetime
    FROM country_club.Members);
...
LEFT JOIN (
    ...
    ON b.facid = f.facid
    WHERE f.name LIKE '%Tennis Court%') AS t
ON b.memid = m.memid
ORDER BY 1,2;
...
LEFT JOIN country_club.Members m
	ON b.memid = m.memid
WHERE SUBSTR(b.starttime, 1, 10) = '2012-09-04'
	AND ((CASE WHEN b.memid = 0 THEN guestcost 
		ELSE membercost END) * b.slots)  > 30
...
ON m.memid = b_sept4.memid
WHERE b_sept4.cost  > 30
ORDER by cost DESC, fac_name;
